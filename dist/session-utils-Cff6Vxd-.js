import { et as consumeRootOptionToken } from "./subsystem-kzdGVyce.js";
import { g as resolveStateDir } from "./paths-BfR2LXbA.js";
import { Ar as resolveDefaultModelForAgent, Er as parseModelRef, Ft as isPathWithinRoot, H as loadConfig, Lt as isWorkspaceRelativeAvatarPath, Mt as isAvatarDataUrl, Nt as isAvatarHttpUrl, Ri as DEFAULT_MODEL, br as inferUniqueProviderFromConfiguredModels, jt as AVATAR_MAX_BYTES, kr as resolveConfiguredModelRef, ur as resolveOpenClawAgentDir, zi as DEFAULT_PROVIDER, zt as resolveAvatarMime } from "./auth-profiles-UpqQjKB-.js";
import { d as resolveAgentWorkspaceDir, f as resolveDefaultAgentId } from "./agent-scope-BDXYqF89.js";
import { C as isCronRunSessionKey, E as parseAgentSessionKey, c as normalizeAgentId, l as normalizeMainKey } from "./session-key-GuEQvqMH.js";
import { r as openBoundaryFileSync } from "./boundary-file-read-CcBHg_z-.js";
import { t as ensureOpenClawModelsJson } from "./models-config-Ct5bZG7t.js";
import { $ as resolveAgentMainSessionKey, E as normalizeSessionDeliveryFields, P as readSessionTitleFieldsFromTranscript, Q as canonicalizeMainSessionAlias, X as resolveFreshSessionTotalTokens, c as loadSessionStore, ot as buildGroupDisplayName, tt as resolveMainSessionKey } from "./sessions-Bh025Q5O.js";
...
function ensureContextWindowCacheLoaded() {
	...
	loadPromise = (async () => {
		...
		try {
			const { discoverAuthStorage, discoverModels } = await import("./pi-model-discovery-CHZFD2LJ.js").then((n) => n.r);
...
function getSessionDefaults(cfg) {
	...
	const contextTokens = cfg.agents?.defaults?.contextTokens ?? lookupContextTokens(resolved.model) ?? 2e5;
	return {
...
function listSessionsFromStore(params) {
	...
	let sessions = Object.entries(store).filter(([key]) => {
		...
		const model = resolvedModel.model ?? "claude-opus-4-6";
		return {
			key,
			entry,
