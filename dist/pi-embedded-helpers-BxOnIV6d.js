import { I as resolvePathViaExistingAncestorSync, O as runExec, S as ensureAgentWorkspace, W as normalizeHyphenSlug, Y as buildAgentMainSessionKey, _ as DEFAULT_HEARTBEAT_FILENAME, b as DEFAULT_TOOLS_FILENAME, ct as normalizeOptionalAccountId, et as normalizeAgentId, f as resolveSessionAgentId, g as DEFAULT_BOOTSTRAP_FILENAME, h as DEFAULT_AGENT_WORKSPACE_DIR, i as resolveAgentConfig, j as openBoundaryFile, m as DEFAULT_AGENTS_FILENAME, nt as resolveAgentIdFromSessionKey, q as DEFAULT_AGENT_ID, tt as normalizeMainKey, v as DEFAULT_IDENTITY_FILENAME, x as DEFAULT_USER_FILENAME, y as DEFAULT_SOUL_FILENAME } from "./run-with-concurrency-CF8VuJbn.js";
import { a as resolveGatewayPort, d as resolveRequiredHomeDir, n as STATE_DIR } from "./paths-DinMprTu.js";
import { E as truncateUtf16Safe, M as defaultRuntime, O as createSubsystemLogger, h as normalizeE164, y as resolveUserPath } from "./logger-BMLgyw16.js";
import { Ar as resolveSecretInputRef, Dr as hasConfiguredSecretInput, Q as loadConfig, W as resolveSecretRefValues, Y as createConfigIO, _t as getBlockedNetworkModeReason, br as isPidAlive, gt as secretRefKey, lr as parseBooleanValue, nr as containsEnvVarReference, qt as CHANNEL_IDS, rt as writeConfigFile, vr as resolveProcessScopedMap, vt as parseDurationMs, xr as formatCliCommand, yr as getProcessStartTime, yt as parseByteSize } from "./model-selection-n7SaaZtn.js";
import { d as getChannelDock, o as normalizeThinkLevel } from "./thinking-B8-468WR.js";
import { r as normalizeChannelId, y as normalizeChatType } from "./plugins-DfC8FIFE.js";
import { t as generateSecureToken } from "./secure-random-D8DfzL9q.js";
import { i as getImageMetadata, m as normalizeMessageChannel, n as buildImageResizeSideGrid, p as listDeliverableMessageChannels, s as resizeToJpeg, t as IMAGE_REDUCE_QUALITY_STEPS } from "./image-ops-cult1iKc.js";
import { $ as DEFAULT_AI_SNAPSHOT_EFFICIENT_MAX_CHARS, A as resolveExistingPathsWithinRoot, B as BrowserTargetAmbiguousError, D as DEFAULT_DOWNLOAD_DIR, F as BrowserProfileNotFoundError, H as toBrowserErrorResponse, I as BrowserProfileUnavailableError, J as withBrowserNavigationPolicy, K as assertBrowserNavigationResultAllowed, L as BrowserResetUnsupportedError, M as resolveWritablePathWithinRoot, N as BrowserConfigurationError, O as DEFAULT_TRACE_DIR, P as BrowserConflictError, R as BrowserResourceExhaustedError, S as CDP_JSON_NEW_TIMEOUT_MS, U as InvalidBrowserNavigationUrlError, V as BrowserValidationError, W as assertBrowserNavigationAllowed, X as normalizeBrowserFormField, Z as isLoopbackHost, a as resolveOpenClawUserDataDir, b as ensureChromeExtensionRelayServer, c as resolveBrowserExecutableForPlatform, et as DEFAULT_AI_SNAPSHOT_MAX_CHARS, f as normalizeCdpWsUrl, g as fetchOk, h as fetchJson, i as launchOpenClawChrome, k as DEFAULT_UPLOAD_DIR, l as captureScreenshot, m as appendCdpPath, n as isChromeCdpReady, nt as DEFAULT_OPENCLAW_BROWSER_COLOR, o as stopOpenClawChrome, p as snapshotAria, q as requiresInspectableBrowserNavigationRedirects, r as isChromeReachable, rt as DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME, u as createTargetViaCdp, v as normalizeCdpHttpBaseForJsonEndpoints, w as resolveCdpReachabilityTimeouts, x as stopChromeExtensionRelayServer, z as BrowserTabNotFoundError } from "./chrome-Dk7SpaMr.js";
import { a as syncSkillsToWorkspace, l as resolveSandboxInputPath, m as sanitizeEnvVars, u as resolveSandboxPath } from "./skills-DSroUgpf.js";
import { t as PATH_ALIAS_POLICIES } from "./path-alias-guards-MbRvgeun.js";
import { n as SsrFBlockedError } from "./proxy-env-DyR9mSSr.js";
import { n as extractErrorCode, r as formatErrorMessage } from "./errors-BSnJC4eK.js";
import { i as saveMediaBuffer, n as ensureMediaDir } from "./store-_BT3dZ3O.js";
import { n as resolveWindowsSpawnProgram, t as materializeWindowsSpawnProgram } from "./windows-spawn-BanZKa46.js";
import { i as writeTextAtomic, r as writeJsonAtomic } from "./json-files-CE9nLZ9c.js";
import { a as resolveSessionTranscriptPathInDir, i as resolveSessionTranscriptPath, n as resolveSessionFilePath, r as resolveSessionFilePathOptions, s as resolveStorePath, t as resolveDefaultSessionStorePath } from "./paths-DcVRZkOJ.js";
import { t as emitSessionTranscriptUpdate } from "./transcript-events-BDSDxU8V.js";
import { t as sanitizeContentBlocksImages } from "./tool-images-BsAYbBkW.js";
...
function trimCredentialToUndefined(value) {
	const trimmed = trimToUndefined$1(value);
...
function resolveGatewayCredentialsFromConfig(params) {
	...
	if (trimToUndefined$1(params.urlOverride) && params.urlOverrideSource !== "env") return {};
	if (trimToUndefined$1(params.urlOverride) && params.urlOverrideSource === "env") return resolveGatewayCredentialsFromValues({
		configToken: void 0,
	...
	const remoteToken = remoteTokenRef ? void 0 : trimToUndefined$1(remote?.token);
	const remotePassword = remotePasswordRef ? void 0 : trimToUndefined$1(remote?.password);
	const localToken = localTokenRef ? void 0 : trimToUndefined$1(params.cfg.gateway?.auth?.token);
	const localPassword = localPasswordRef ? void 0 : trimToUndefined$1(params.cfg.gateway?.auth?.password);
...
async function loadPwAiModule(mode) {
	try {
		return await import("./pw-ai-C5pDZBuJ.js");
	} catch (err) {
...
function resolveDefaultSnapshotFormat(params) {
	...
	if (getBrowserProfileCapabilities(params.profile).mode === "local-extension-relay") return "aria";
	return params.hasPlaywright ? "ai" : "aria";
}
function shouldUsePlaywrightForScreenshot(params) {
	return getBrowserProfileCapabilities(params.profile).requiresRelay || !params.wsUrl || Boolean(params.ref) || Boolean(params.element);
}
...
async function resolveTargetIdAfterNavigate(opts) {
	...
	try {
		const pickReplacement = (tabs) => {
			...
			if (tabs.length === 1) return tabs[0]?.targetId ?? opts.oldTargetId;
...
function resolveBrowserConfig(cfg, rootConfig) {
	...
	const profiles = ensureDefaultChromeExtensionProfile(ensureDefaultProfile(cfg?.profiles, defaultColor, legacyCdpPort, cdpPortRangeStart, legacyCdpUrl), controlPort);
	const cdpProtocol = cdpInfo.parsed.protocol === "https:" ? "https" : "http";
	const defaultProfile = defaultProfileFromConfig ?? (profiles["openclaw"] ? "openclaw" : profiles["openclaw"] ? "openclaw" : "chrome");
	const extraArgs = Array.isArray(cfg?.extraArgs) ? cfg.extraArgs.filter((a) => typeof a === "string" && a.trim().length > 0) : [];
	const ssrfPolicy = resolveBrowserSsrFPolicy(cfg);
	const relayBindHost = cfg?.relayBindHost?.trim() || void 0;
	return {
		enabled,
		evaluateEnabled,
		controlPort,
...
function createBrowserProfilesService(ctx) {
	...
	const createProfile = async (params) => {
		...
		if (rawCdpUrl) {
			...
			if (driver === "extension") {
				if (!isLoopbackHost(parsed.parsed.hostname)) throw new BrowserValidationError(`driver=extension requires a loopback cdpUrl host, got: ${parsed.parsed.hostname}`);
				if (parsed.parsed.protocol !== "http:" && parsed.parsed.protocol !== "https:") throw new BrowserValidationError(`driver=extension requires an http(s) cdpUrl, got: ${parsed.parsed.protocol.replace(":", "")}`);
			}
			profileConfig = {
				cdpUrl: parsed.normalized,
				...driver ? { driver } : {},
			...
			if (driver === "extension") throw new BrowserValidationError("driver=extension requires an explicit loopback cdpUrl");
	...
	const deleteProfile = async (nameRaw) => {
		...
		if (name === (cfg.browser?.defaultProfile ?? "openclaw")) throw new BrowserValidationError(`cannot delete the default profile "${name}"; change browser.defaultProfile first`);
		let deleted = false;
		const state = ctx.state();
...
async function withBasicProfileRoute(params) {
	...
	try {
		...
		if (mapped) return jsonError(params.res, mapped.status, mapped.message);
...
function createProfileAvailability({ opts, profile, state, getProfileState, setProfileRunning }) {
	...
	const closePlaywrightBrowserConnectionForProfile = async (cdpUrl) => {
		try {
			await (await import("./pw-ai-C5pDZBuJ.js")).closePlaywrightBrowserConnection(cdpUrl ? { cdpUrl } : void 0);
		} catch {}
	};
	const reconcileProfileRuntime = async () => {
		...
		if (previousProfile.driver === "extension") await stopChromeExtensionRelayServer({ cdpUrl: previousProfile.cdpUrl }).catch(() => false);
	...
	const ensureBrowserAvailable = async () => {
		...
		if (isExtension && remoteCdp) throw new BrowserConfigurationError(`Profile "${profile.name}" uses driver=extension but cdpUrl is not loopback (${profile.cdpUrl}).`);
		if (isExtension) {
			if (!httpReachable) {
				...
				if (!await isHttpReachable(1200)) throw new BrowserProfileUnavailableError(`Chrome extension relay for profile "${profile.name}" is not reachable at ${profile.cdpUrl}.`);
			}
			return;
		}
		if (!httpReachable) {
	...
	const stopRunningBrowser = async () => {
		await reconcileProfileRuntime();
		if (capabilities.requiresRelay) return { stopped: await stopChromeExtensionRelayServer({ cdpUrl: profile.cdpUrl }) };
		const profileState = getProfileState();
...
function createProfileSelectionOps({ profile, getProfileState, ensureBrowserAvailable, listTabs, openTab }) {
	...
	const ensureTabAvailable = async (targetId) => {
		...
		if (tabs1.length === 0) if (capabilities.requiresAttachedTab) {
			if (profileState.lastTargetId?.trim()) {
				const deadlineAt = Date.now() + 3e3;
				while (tabs1.length === 0 && Date.now() < deadlineAt) {
					await new Promise((resolve) => setTimeout(resolve, 200));
					tabs1 = await listTabs();
				}
			}
			if (tabs1.length === 0) throw new BrowserTabNotFoundError(`tab not found (no attached Chrome tabs for profile "${profile.name}"). Click the OpenClaw Browser Relay toolbar icon on the tab you want to control (badge ON).`);
...
var SandboxFsPathGuard = class {
	...
	async resolveAnchoredSandboxEntry(target) {
		...
		return {
			canonicalParentPath: await this.resolveCanonicalContainerPath({
...
function buildWriteCommitPlan(target, tempPath) {
	return {
		checks: [{
			target,
			options: {
...
function buildMkdirpPlan(target, anchoredTarget) {
	return {
		checks: [{
			target,
			options: {
...
function buildRemovePlan(params) {
...
var SandboxFsBridgeImpl = class {
	...
	async remove(params) {
		...
		const anchoredTarget = await this.pathGuard.resolveAnchoredSandboxEntry(target);
		await this.runPlannedCommand(buildRemovePlan({
			target,
			anchoredTarget,
	...
	async rename(params) {
		...
		await this.runPlannedCommand(buildRenamePlan({
			from,
			to,
			anchoredFrom,
			anchoredTo
...
function resolveMainSessionKey(cfg) {
	...
	return buildAgentMainSessionKey({
		agentId: normalizeAgentId(agents.find((agent) => agent?.default)?.id ?? agents[0]?.id ?? "main"),
...
function resolveSessionResetPolicy(params) {
	...
	} else if (mode === "idle") idleMinutes = 60;
	return {
		mode,
		atHour,
		idleMinutes
...
//#endregion
//#region src/discord/session-key-normalization.ts
function normalizeExplicitDiscordSessionKey(sessionKey, ctx) {
	let normalized = sessionKey.trim().toLowerCase();
	if (normalizeChatType(ctx.ChatType) !== "direct") return normalized;
...
async function resolveSandboxContext(params) {
	...
	const browser = await ensureSandboxBrowser({
		...
		agentWorkspaceDir,
		cfg: resolvedCfg,
		evaluateEnabled: params.config?.browser?.evaluateEnabled ?? true,
...
//#endregion
export { resolveCacheTtlMs as $, normalizeToolName as $t, isRawApiErrorPayload as A, deriveSessionMetaPatch as At, resolveMirroredTranscriptText as B, ensureBrowserControlAuth as Bt, isCloudCodeAssistFormatError as C, DEFAULT_RESET_TRIGGERS as Ct, isFailoverErrorMessage as D, canonicalizeMainSessionAlias as Dt, isFailoverAssistantError as E, setSessionRuntimeModel as Et, isTimeoutErrorMessage as F, createBrowserRouteContext as Ft, readSessionUpdatedAt as G, resolveSandboxConfigForAgent as Gt, resolveAndPersistSessionFile as H, GatewaySecretRefUnavailableError as Ht, ensureSandboxWorkspaceForSession as I, registerBrowserRoutes as It, saveSessionStore as J, collectExplicitAllowlist as Jt, recordSessionMetaFromInbound as K, applyOwnerOnlyToolPolicy as Kt, resolveSandboxContext as L, resolveBrowserConfig as Lt, parseImageDimensionError as M, resolveGroupSessionKey as Mt, parseImageSizeError as N, acquireSessionWriteLock as Nt, isLikelyContextOverflowError as O, resolveExplicitAgentSessionKey as Ot, sanitizeUserFacingText as P, resolveSessionLockMaxHoldFromTimeout as Pt, isCacheEnabled as Q, expandToolGroups as Qt, resolveSandboxRuntimeStatus as R, resolveProfile as Rt, isBillingAssistantError as S, resolveThreadFlag as St, isContextOverflowError as T, resolveFreshSessionTotalTokens as Tt, extractDeliveryInfo as U, resolveGatewayCredentialsFromConfig as Ut, resolveSessionTranscriptFile as V, resolveBrowserControlAuth as Vt, loadSessionStore as W, trimToUndefined$1 as Wt, updateSessionStore as X, mergeAlsoAllowPolicy as Xt, updateLastRoute as Y, expandPolicyWithPluginGroups as Yt, updateSessionStoreEntry as Z, stripPluginOnlyAllowlist as Zt, formatAssistantErrorText as _, resolveSessionKey as _t, isMessagingToolDuplicateNormalized as a, resolveBootstrapMaxChars as an, normalizeSessionDeliveryFields as at, getApiErrorPayloadFingerprint as b, resolveSessionResetPolicy as bt, extractToolCallsFromAssistant as c, sanitizeGoogleTurnOrdering as cn, capArrayByJsonBytes as ct, downgradeOpenAIFunctionCallReasoningPairs as d, parseInlineDirectives as dt, resolveToolProfilePolicy as en, parseFiniteNumber as et, downgradeOpenAIReasoningBlocks as f, INPUT_PROVENANCE_KIND_VALUES as ft, classifyFailoverReasonFromHttpStatus as g, jsonUtf8Bytes as gt, classifyFailoverReason as h, normalizeInputProvenance as ht, isMessagingToolDuplicate as i, ensureSessionHeader$1 as in, normalizeDeliveryContext as it, isTransientHttpError as j, resolveConversationLabel as jt, isRateLimitAssistantError as k, resolveMainSessionKey as kt, extractToolResultId as l, countToolResults as lt, BILLING_ERROR_USER_MESSAGE as m, hasInterSessionUserProvenance as mt, validateGeminiTurns as n, matchesAnyGlobPattern as nn, deliveryContextKey as nt, normalizeTextForComparison as o, resolveBootstrapPromptTruncationWarningMode as on, normalizeAccountId as ot, isGoogleModelApi as p, applyInputProvenanceToUserMessage as pt, resolveSessionStoreEntry as q, buildPluginToolGroups as qt, pickFallbackThinkingLevel as r, buildBootstrapContextFiles as rn, mergeDeliveryContext as rt, sanitizeSessionMessagesImages as s, resolveBootstrapTotalMaxChars as sn, archiveSessionTranscripts as st, validateAnthropicTurns as t, compileGlobPatterns as tn, deliveryContextFromSession as tt, sanitizeToolCallIdsForCloudCodeAssist as u, extractToolCallNames as ut, formatBillingErrorMessage as v, evaluateSessionFreshness as vt, isCompactionFailureError as w, mergeSessionEntry as wt, isAuthAssistantError as x, resolveSessionResetType as xt, formatRawAssistantErrorForUi as y, resolveChannelResetConfig as yt, appendAssistantMessageToSessionTranscript as z, getBridgeAuthForPort as zt };
