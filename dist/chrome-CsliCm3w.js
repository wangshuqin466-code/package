import { T as resolvePreferredOpenClawTmpDir, t as createSubsystemLogger } from "./subsystem-C9Gk4AAH.js";
import { D as isNotFoundPathError, O as isPathInside } from "./workspace-Cn3fdLBW.js";
import { a as CONFIG_DIR } from "./logger-CJbXRTpA.js";
import { $ as loadConfig, Dt as isLoopbackIpAddress, Er as resolveSecretInputRef, G as resolveSecretRefValues, Ot as isPrivateOrLoopbackIpAddress, Tr as normalizeSecretInputString, _t as secretRefKey, kt as normalizeIpAddress } from "./model-selection-C8ExQCsd.js";
import { n as SsrFBlockedError, o as isPrivateNetworkAllowedByPolicy, s as resolvePinnedHostnameWithPolicy, t as hasProxyEnvConfigured } from "./proxy-env-C2KrBwJo.js";
import { a as isErrno } from "./errors-CH6uzT9l.js";
import { i as openFileWithinRoot, t as SafeOpenError } from "./fs-safe-C6qEGKLE.js";
...
var BrowserConfigurationError = class extends BrowserError {
	constructor(message, options) {
		super(message, 400, options);
	}
};
...
async function ensureChromeExtensionRelayServer(opts) {
	...
	if (existing) if (existing.server.bindHost !== bindHost) await existing.server.stop();
	else return existing.server;
	const inFlight = relayInitByPort.get(info.port);
	if (inFlight) {
		const server = await inFlight;
		if (server.bindHost === bindHost) return server;
		await server.stop();
	}
...
//#endregion
export { DEFAULT_AI_SNAPSHOT_EFFICIENT_MAX_CHARS as $, resolveExistingPathsWithinRoot as A, BrowserTargetAmbiguousError as B, PROFILE_ATTACH_RETRY_TIMEOUT_MS as C, DEFAULT_DOWNLOAD_DIR as D, rawDataToString as E, BrowserProfileNotFoundError as F, assertBrowserNavigationRedirectChainAllowed as G, toBrowserErrorResponse as H, BrowserProfileUnavailableError as I, withBrowserNavigationPolicy as J, assertBrowserNavigationResultAllowed as K, BrowserResetUnsupportedError as L, resolveWritablePathWithinRoot as M, BrowserConfigurationError as N, DEFAULT_TRACE_DIR as O, BrowserConflictError as P, isSecureWebSocketUrl as Q, BrowserResourceExhaustedError as R, CDP_JSON_NEW_TIMEOUT_MS as S, withNoProxyForCdpUrl as T, InvalidBrowserNavigationUrlError as U, BrowserValidationError as V, assertBrowserNavigationAllowed as W, normalizeBrowserFormField as X, DEFAULT_FILL_FIELD_TYPE as Y, isLoopbackHost as Z, getHeadersWithAuth as _, resolveOpenClawUserDataDir as a, ensureChromeExtensionRelayServer as b, resolveBrowserExecutableForPlatform as c, formatAriaSnapshot as d, DEFAULT_AI_SNAPSHOT_MAX_CHARS as et, normalizeCdpWsUrl as f, fetchOk as g, fetchJson as h, launchOpenClawChrome as i, resolveStrictExistingPathsWithinRoot as j, DEFAULT_UPLOAD_DIR as k, captureScreenshot as l, appendCdpPath as m, isChromeCdpReady as n, DEFAULT_OPENCLAW_BROWSER_COLOR as nt, stopOpenClawChrome as o, snapshotAria as p, requiresInspectableBrowserNavigationRedirects as q, isChromeReachable as r, DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME as rt, resolveLsofCommandSync as s, getChromeWebSocketUrl as t, DEFAULT_BROWSER_DEFAULT_PROFILE_NAME as tt, createTargetViaCdp as u, normalizeCdpHttpBaseForJsonEndpoints as v, resolveCdpReachabilityTimeouts as w, stopChromeExtensionRelayServer as x, withCdpSocket as y, BrowserTabNotFoundError as z };
